...
[ ] QC execution page
Let's double click on @ui/src/pages/qc/QCExecution.tsx. these executions will be performed on a vertical/horizontal tablet. Its very important that our lay out look really good under those conditions. This page in particualr can mostly lack header UI elements (we can keep a minimal back button and nothing else). It should display a carousel of images for the references and guidance images (two image carousels sharing half the screens width roughly). The description should be placed smartly so as to be grouped with the buttons inteliigently. 