...
from app.schemas.analytics import PanelTaskHistoryPause, PanelTaskHistoryRow
...
def get_panel_task_history(
    ...
    rows = list(
        db.execute(stmt.offset(offset).limit(limit)).all()
    )
    if not rows:
        return []
    ...
    for _, _, _, panel_definition, _, _, _, _, _ in rows:
    ...
    response_rows: list[PanelTaskHistoryRow] = []
    for (
        instance,
        task_def,
        _panel_unit,
        ...
        response_rows.append(
            PanelTaskHistoryRow(
                ...
                house_sub_type_name=house_sub_type.name
                if house_sub_type
                else None,
